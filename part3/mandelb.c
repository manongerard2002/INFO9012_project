...
void circularBufferEnqueueToCompute(circularBuffer *cbuf, int futureTime, double scale, double cx, double cy, double angle);
void circularBufferChooseNextToCompute(circularBuffer *cbuf, unsigned *index);
void circularBufferUpdateComputed(circularBuffer *cbuf, unsigned idx);
void circularBufferDequeue(circularBuffer *cbuf, unsigned *index);
...
void *producer(void *arg)
{
  while (true)
  {
    ...
    circularBufferChooseNextToCompute(cbuf,  &idx);
...
int main(int argc, char **argv)
{
 ...
 skip_usage:
  ...
  if (sem_init(&semN, 0, 0))
    error(-1, 0, "Error initializing semaphore N");
  if (sem_init(&semE, 0, 16-1))
    error(-1, 0, "Error initializing semaphore E");
  for (unsigned i = 0; i < 16; i++)
  {
    sem_t semA_idx = chooseSemA(i);
    if (sem_init(&semA_idx, 0, 0))
      error(-1, 0, "Error initializing semaphores A");
  }
  ...
  {
    circularBufferEnqueueToCompute(cbuf, futureTime, scale, cx, cy, angle);
  }
  long ms = 0;
  ...
  while (1)
    {
      ...
      circularBufferDequeue(cbuf, &idx);
 ...
 exiting:
  ...
  // destroy semaphores
  if (sem_destroy(&semE))
    error(-1, 0, "Error destroying semaphore E");
  if (sem_destroy(&semN))
    error(-1, 0, "Error destroying semaphore N");
  for (unsigned i = 0; i < 16; i++)
  {
    sem_t semA_idx = chooseSemA(i);
    if (sem_destroy(&semA_idx))
      error(-1, 0, "Error destroying semaphores A");
  }
